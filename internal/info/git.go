...
func FetchGitInfo(root string, colorCode int) []string {
	commits, last_commit, contributors, err := fetchGit(root);
	if !err {
	...
	res[2] = utils.ColorText("Last commit: ", colorCode) + last_commit
	res[3] = utils.ColorText("Number of contributors: ", colorCode) + strconv.Itoa(contributors) 
	return res
}

func fetchGit(root string) (int, string, int, bool) {
	...
	return numCommits, strings.TrimSpace(lastCommit), contributors, true
}